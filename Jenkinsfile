  ...
  // }
    ...
    steps {
       def outputFilePath = "${env.WORKSPACE}"
        ...
        bat 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v C:\Users\AsmaM\trrufellhog_project\PHP-MySQL-CRUD-Operation-Employee-Management-CRUD:/output aquasec/trivy:latest image --format table -o /output/report-trivy.txt my-php-app:latest'
    }
}
}
}
